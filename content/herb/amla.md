---
title: "Amla"
slug: "amla"
kind: "herb"
section: "Ayurveda"
category: "Herbs"
subcategory: "Single-Herbs"
group: "Rasayana-Immunity"
answer: "Amla (Phyllanthus emblica (syn. Emblica officinalis)) is classified in Ayurveda as Rasayana (Rejuvenative), Tridoshahara, from the Phyllanthaceae family. Parts used: fruit, seed, bark, leaf. One of the three fruits in Triphala. No dedicated WHO monograph identified for Phyllanthus emblica specifically."
botanical: "Phyllanthus emblica (syn. Emblica officinalis)"
family: "Phyllanthaceae"
sanskrit: ""
ayurvedicCategory: "Rasayana (Rejuvenative), Tridoshahara"
whoStatus: "No dedicated WHO monograph identified for Phyllanthus emblica specifically. Listed in the Pharmacopoeia of the People's Republic of China (as Phyllanthi Fructus), Ayurvedic Pharmacopoeia of India, and referenced in multiple traditional medicine pharmacopeias. Recognized in Unani (Greco-Arab) and Ayurvedic systems of medicine. Over 180 compounds characterized with extensive pharmacological documentation."
aliases: ["Amla", "Amalaki", "Dhatri", "Phyllanthus emblica", "Phyllanthus emblica (syn. Emblica officinalis)", "Emblica officinalis", "Indian Gooseberry"]
tags: ["ayurveda", "herb", "single-herb", "rasayana-immunity", "rasayana", "pharmacopoeia-listed", "clinical-evidence", "safety-concern"]
sources: ["Amidha Ayurveda Herb Database (700+ herbs, CC-BY-4.0)", "AgeAyurveda Product Catalog", "Classical Ayurvedic texts (Charaka Samhita, Sushruta Samhita, Bhavaprakasha Nighantu, Ashtanga Hridaya)", "Web research (clinical trials, WHO monographs, safety databases)", "Ayurvedic Pharmacopoeia of India"]
productHints: ["Immuno Plus", "Acid Relief", "Chyawanprash Avaleha", "Chyawan Cap", "Grow and Glow Hair Oil", "Nutrijam"]
words: 1153
srcRel: "Ayurveda/Herbs/Single-Herbs/Rasayana-Immunity/Amla/meta-analysis.md"
---
## Names and identification

| Language | Name |
|----------|------|
| English | Amla |
| Common Names | Indian Gooseberry, Amalaki |
| Hindi | Amla, Aonla |
| Sanskrit | Amalaki, Dhatri |
| Latin/Botanical | *Phyllanthus emblica (syn. Emblica officinalis)* |

## Parts Used

- Fruit
- Seed
- Bark
- Leaf

## Dosha Effects

| Dosha | Effect |
|-------|--------|
| **Vata** | Decreases |
| **Kapha** | Decreases |
| **Pitta** | Decreases |

## Key Phytochemical Constituents

- Vitamin C (ascorbic acid)
- Gallic acid
- Ellagic acid
- [Tannins](/reference/tannins/) (emblicanin A & B)
- Phyllemblin

## Therapeutic Actions (Karma)

- [Antioxidant](/reference/antioxidant/) (highest natural Vitamin C)
- Rasayana (rejuvenative)
- [Hepatoprotective](/reference/hepatoprotective/)
- Anti-diabetic
- Immunomodulator
- Hair tonic

## How does it work?

- Active compounds: Over 180 identified compounds including gallic acid, ellagic acid, chebulinic acid, tannins, [flavonoids](/reference/flavonoids/), [terpenoids](/glossary/compounds-t-z/#terpenoids), [polysaccharides](/glossary/compounds-o-q/#polysaccharides).
- Antioxidant: Potent DPPH radical scavenging and reducing power; super antioxidant properties confirmed by genomic analysis.
- Anti-inflammatory: RBC membrane stabilization; 15-lipoxygenase (15-LOX) inhibition; regulates multiple inflammopathic checkpoints.
- Anti-diabetic (multi-mechanism): Inhibition of digestive [enzymes](/glossary/compounds-d-g/#enzymes) (alpha-amylase, alpha-glucosidase), stimulation of glycogen storage, activation of insulin signaling, inhibition of advanced glycation end-products (AGEs).
- Adipogenesis inhibition: Tannin content prevents adipogenesis and increases [glucose](/glossary/compounds-d-g/#glucose) uptake by enhancing peripheral insulin sensitivity.
- Neuroprotection: Targets MAPK3, SRC, MAPK1; modulates neuroactive ligand-receptor interactions and serotonergic synapses.
- Hepatoprotective and nephroprotective: Cytoprotective effects on liver and kidney tissues.
- [Immunomodulatory](/reference/immunomodulatory/) and antiviral: Broad-spectrum immune support and documented antiviral activity.

## How is it used traditionally?

One of the three fruits in [Triphala](/herb/triphala/). Classified as best among Rasayana (rejuvenatives) and Vayasthapana (anti-aging) herbs by Charaka. Key ingredient in [Chyawanprash](/formulation/chyawanprash/).

## Where is it described in the classical texts?

- Charaka Samhita (Vayasthapana group - anti-aging)
- Sushruta Samhita
- Bhavaprakasha Nighantu
- Ashtanga Hridaya

## What do recent clinical trials show?

- Setayesh L, Haghighat N, Rasaei N and others 2023. [The impact of Emblica Officinalis (Amla) on lipid profile, glucose, and C-reactive protein: A systematic review and meta-analysis of randomized controlled trials](https://pubmed.ncbi.nlm.nih.gov/36934568/). *Diabetes & metabolic syndrome*. PMID [36934568](https://pubmed.ncbi.nlm.nih.gov/36934568/) · [doi:10.1016/j.dsx.2023.102729](https://doi.org/10.1016/j.dsx.2023.102729)  
  Meta-analysis of RCTs showed amla supplementation significantly improved lipid profiles (reduced LDL, elevated HDL), lowered glucose levels, and reduced C-reactive protein in adults.
- Varnasseri M, Siahpoosh A, Hoseinynejad K and others 2022. [The effects of add-on therapy of Phyllanthus Emblica (Amla) on laboratory confirmed COVID-19 Cases: A randomized, double-blind, controlled trial](https://pubmed.ncbi.nlm.nih.gov/35093510/). *Complementary therapies in medicine*. PMID [35093510](https://pubmed.ncbi.nlm.nih.gov/35093510/) · [doi:10.1016/j.ctim.2022.102808](https://doi.org/10.1016/j.ctim.2022.102808)  
  Amla add-on therapy showed significant improvements in fever, cough severity, shortness of breath, myalgia, increased oxygen saturation, and reduced lung involvement on CT in COVID-19 patients.
- Akhbari M, Firooz A, Rahimi R and others 2024. [The effect of an oral product containing Amla fruit (Phyllanthus emblica L.) on female androgenetic alopecia: A randomized controlled trial](https://pubmed.ncbi.nlm.nih.gov/37487962/). *Journal of ethnopharmacology*. PMID [37487962](https://pubmed.ncbi.nlm.nih.gov/37487962/) · [doi:10.1016/j.jep.2023.116958](https://doi.org/10.1016/j.jep.2023.116958)  
  Triple-blind RCT with 60 women showed amla syrup (3x daily for 12 weeks) helped treat androgenic hair loss and increased the anagen (growth) phase of hair follicles.
- Kapoor MP, Suzuki K, Derek T and others 2020. [Clinical evaluation of Emblica Officinalis Gatertn (Amla) in healthy human subjects: Health benefits and safety results from a randomized, double-blind, crossover placebo-controlled study](https://pubmed.ncbi.nlm.nih.gov/31890983/). *Contemporary clinical trials communications*. PMID [31890983](https://pubmed.ncbi.nlm.nih.gov/31890983/) · [doi:10.1016/j.conctc.2019.100499](https://doi.org/10.1016/j.conctc.2019.100499)  
  500 mg/day amla for 18 weeks improved blood fluidity, lowered von Willebrand factor, reduced oxidative stress markers, improved HDL, and lowered LDL cholesterol.
- Xiao L, Chen W, Guo W and others 2025. [Exploring the mechanism of action of Phyllanthus emblica in the treatment of epilepsy based on network pharmacology and molecular docking](https://pubmed.ncbi.nlm.nih.gov/39960946/). *Medicine*. PMID [39960946](https://pubmed.ncbi.nlm.nih.gov/39960946/) · [doi:10.1097/MD.0000000000041414](https://doi.org/10.1097/MD.0000000000041414)  
  Network pharmacology identified neuroactive ligand-receptor interactions, nitrogen metabolism, and serotonergic synapses as key pathways. Molecular docking showed strong binding to MAPK3, SRC, and MAPK1 targets.

*1 further claim previously listed here could not be traced to a published paper and has been removed. An absence here means we could not identify the source, not that no work exists.*

## Recent safety updates

- Generally nontoxic: Artemia nauplii bioassays confirmed all extracts nontoxic (2024-2025 studies).
- GI effects: Cramping or diarrhea possible at high doses (>5 g/day).
- Drug interactions with anticoagulants: Possible interaction with warfarin and clopidogrel; pharmacodynamic study documented with clopidogrel and ecosprin in Type II diabetes patients.
- Hypoglycemia risk: May interact with anti-diabetic medications; blood sugar monitoring recommended.
- Iron absorption: High tannin content may reduce non-heme iron uptake; caution in iron-deficient individuals.
- Surgery precaution: May affect bleeding risk and drug metabolism; caution before medical procedures.
- Antibacterial synergy: Phyllanthus emblica extracts show synergistic interactions with certain antibiotics (2025 study), which could affect antibiotic dosing.

## Recommended Dosage

Fresh fruit: 1-2 fruits/day; Powder (Churna): 3-6g/day; Juice: 10-20ml/day

## Safety, contraindications and cautions

- May lower blood sugar (monitor with diabetes medications)
- Iron absorption enhancement (caution with hemochromatosis)

## What is it made of?

**Part(s) Analyzed:** Fruit

### Tannins (Hydrolyzable)

- Emblicanin A (2-5%)
- Emblicanin B
- Punigluconin
- Pedunculagin
- Corilagin
- Geraniin
- Ellagitannins

### Phenolic Acids

- Gallic acid (1-3%)
- Ellagic acid
- Chebulic acid
- Chebulagic acid

### Flavonoids

- Quercetin
- Kaempferol
- Rutin

### Vitamins

- Ascorbic acid (Vitamin C, 600-1800 mg/100g fresh fruit)
- Vitamin C (most stable natural form due to tannin complex)

### Nutritional Profile

- **Vitamin C:** 600-1800 mg/100g fresh fruit
- **Iron:** 1.2 mg/100g
- **Calcium:** 50 mg/100g
- **Phosphorus:** 21 mg/100g
- **Fiber:** 3.4%

**Analytical Methods:** HPLC-PDA (polyphenols), Titration (Vitamin C), UV spectrophotometry

## Dosage forms and preparation

**Dosage Forms:** Churna (powder), Swarasa (fresh juice), Capsule, Tablet, Avaleha/[Chyawanprash](/formulation/chyawanprash/) (confection), [Amalaki Rasayana](/formulation/amalaki-rasayana/), [Triphala](/herb/triphala/) component, Murabba (preserve), Taila (hair oil)

**Standard Dosage:** 3-6g powder twice daily; 15-30ml fresh juice; 500mg-1g extract capsule twice daily; 1-2 tsp Chyawanprash daily

**Bioavailability:** Excellent bioavailability due to natural vitamin C content (ascorbic acid is well absorbed, but Amla vitamin C is more stable due to tannin complexation). Gallic acid and ellagic acid have moderate oral bioavailability (25-30%). [Ellagitannins](/glossary/compounds-d-g/#ellagitannins) are metabolized to urolithins by colonic microbiota. The tannin-vitamin C complex provides sustained release of vitamin C (higher AUC than synthetic ascorbic acid). Emblicanin A and B show good absorption.

**Optimal Timing:** Morning on empty stomach for Rasayana effects; with meals for iron absorption enhancement; at bedtime with warm milk for rejuvenation

**Standardized Extract:** Standardized to minimum 40% tannins (as gallic acid equivalent) OR minimum 20% vitamin C (natural). Low-tannin extract for palatability: 10% tannins, 30% vitamin C. Emblicanin-enriched extract: minimum 10% emblicanins A+B.

**Shelf Life:** 2 years (powder); 3 years (tablets/capsules); 2 years (Chyawanprash); 1 year (preserved juice); 18 months (Murabba, refrigerated)

**Storage:** Airtight containers, cool and dry place below 25°C. Protect from light (photo-degradation of vitamin C and tannins). Hygroscopic powder requires desiccant in packaging.

**Marker Compounds:** Gallic acid, Ellagic acid, Emblicanin A, Emblicanin B, Phyllemblin, [Corilagin](/glossary/compounds-c-d/#corilagin), Chebulagic acid, Ascorbic acid, Punigluconin

### Extraction Methods

- Aqueous extraction (hot water, 80°C, 2 hours)
- Hydroalcoholic extraction (50:50 ethanol-water)
- Spray drying of juice concentrate
- Supercritical CO2 for tannin-enriched extract
- Traditional Swarasa (fresh juice expression and preservation with self-generated preservatives)

### Synergistic Combinations

- With [Haritaki](/herb/haritaki/) and Vibhitaki in Triphala (synergistic antioxidant and digestive action)
- With [Ashwagandha](/herb/ashwagandha/) in Chyawanprash for Rasayana (rejuvenation)
- With [Pippali](/herb/pippali/) for Dhatri Rasayana (iron absorption enhancement)
- With [Yashtimadhu](/herb/yashtimadhu/) for gastric mucosal protection
- With Bhringaraja for hair health
