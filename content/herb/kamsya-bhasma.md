---
title: "Kamsya Bhasma"
slug: "kamsya-bhasma"
kind: "herb"
section: "Ayurveda"
category: "Others"
subcategory: "Mineral-Metal-Preparations"
group: "Bhasmas"
answer: "Kamsya Bhasma is a substance used in the Ayurvedic materia medica. Usual dose: 125–250 mg. Confirmed that Kamsya formulations are beneficial in Krimi (worm infestations) and Kushta (skin diseases). Therapeutic potentials of metals in ancient India: A review through Charaka Samhita (2011): Historical review confirmed bronze's use since Charaka period for therapeutic instruments and preparations."
botanical: ""
family: ""
sanskrit: ""
ayurvedicCategory: ""
whoStatus: ""
aliases: ["Kamsya Bhasma", "KamsyaBhasma"]
tags: ["ayurveda", "bhasma", "pharmacopoeia-listed", "clinical-evidence", "safety-concern"]
sources: ["Amidha Ayurveda Herb Database (700+ herbs, CC-BY-4.0)", "Web research (clinical trials, WHO monographs, safety databases)", "Ayurvedic Pharmacopoeia of India"]
productHints: []
words: 441
srcRel: "Ayurveda/Others/Mineral-Metal-Preparations/Bhasmas/Kamsya-Bhasma/meta-analysis.md"
---
## Names and identification

| Language | Name |
|----------|------|
| English | [Kamsya Bhasma](/ayurveda-atlas/herb/kamsya-bhasma/) |

## Key Phytochemical Constituents

- Copper oxide (CuO) from copper component
- Tin oxide (SnO2) from tin component
- Mixed copper-tin oxide phases
- Trace zinc, iron from alloy impurities
- Organic compounds from herbal processing media

## How does it work?

- Krimighna (anti-parasitic) action through copper and tin oxide-mediated disruption of parasitic cell membranes and metabolic processes
- Kushthahara (anti-dermatological) effect through combined [antimicrobial](/ayurveda-atlas/reference/antimicrobial/) and anti-inflammatory properties of copper-tin oxide nanoparticles on skin lesions
- Netra Roga (eye disease) management through cooling (Sheeta Virya) properties of bronze compounds, reducing Pitta-mediated ocular inflammation
- Medohara (anti-obesity) action through metabolic regulation and lipid metabolism modulation attributed to copper-zinc enzyme cofactor effects

## Which traditional uses are supported by research?

- Worm infestations (Krimi Roga): Anti-parasitic properties of copper compounds provide mechanistic support for traditional use
- Skin diseases (Kushtha): Combined antimicrobial properties of copper and tin oxides support traditional dermatological applications
- Eye disorders (Netra Roga): Historical use of Kansa instruments in eye treatments documented since Charaka period, supported by anti-inflammatory copper properties

## What do recent clinical trials show?

- **A Brief Review of Kamsya (Bronze) Through Ayurvedic Literature** (2023): Comprehensive literature review documented traditional uses, preparation methods, and pharmacological properties. Confirmed that Kamsya formulations are beneficial in Krimi (worm infestations) and Kushta (skin diseases).
- **Therapeutic potentials of metals in ancient India: A review through Charaka Samhita** (2011): Historical review confirmed bronze's use since Charaka period for therapeutic instruments and preparations. Cooling properties of Kansa utilized in eye treatments and Pitta-reducing formulations.
- **Exploring the Potential of Ayurvedic Bhasmas as nanomedicine** (2023): Bhasma nano-particles (1-2 microns) in [Kamsya Bhasma](/ayurveda-atlas/herb/kamsya-bhasma/) aid biological systems in assimilation and absorption. The nano-particulate form is considered harmless at therapeutic levels.

## Recent safety updates

- SAFE AT THERAPEUTIC DOSES: 60-120 mg dose range considered safe when prepared per classical guidelines
- COPPER TOXICITY RISK: Given 80% copper content in source material, copper toxicity symptoms (nausea, vomiting, hepatotoxicity) possible with overdose
- LIMITED MODERN SAFETY DATA: Fewer toxicological studies available compared to single-metal Bhasmas. Most evidence is traditional/empirical.
- ALLOY STANDARDIZATION: Variation in copper:tin ratio across commercial bronze sources may affect both efficacy and safety. Only traditional bell metal (Kansa) ratio recommended.

## What is it made of?

### Mineral/Elemental Profile

- **Copper-tin alloy (Cu-Sn):** Major component (bell metal)
- **Copper (Cu):** ~78%
- **Tin (Sn):** ~22%

**Analytical Methods:** XRD, ICP-OES, SEM-EDS

## Dosage forms and preparation

**Dosage Forms:** Bhasma

**Standard Dosage:** 125–250 mg

**Bioavailability:** Good; bronze converted to bioavailable copper-tin oxide nanoparticles

**Optimal Timing:** With honey; before meals for digestive conditions

**Standardized Extract:** Calcined bronze bhasma

**Shelf Life:** Indefinite

**Storage:** Airtight container

**Marker Compounds:** Copper oxide, Tin oxide, Trace zinc

**Quality Parameters:** Standard bhasma tests, copper content within therapeutic range, no free metal

**Vehicle (Anupana):** Honey or [Triphala](/ayurveda-atlas/herb/triphala/) Kwatha
