---
title: "Ankola"
slug: "ankola"
kind: "herb"
section: "Ayurveda"
category: "Herbs"
subcategory: "Single-Herbs"
group: "General-Herbs"
answer: "Ankola (Alangium salviifolium (L.f.) Wangerin) is a plant used in Ayurveda, from the Cornaceae (Alangiaceae) family. Usual dose: 1-3g powder twice daily (internal, with caution); external oil/paste as needed. Internal use only under practitioner supervision. Not listed in WHO monographs on selected medicinal plants; documented in Indian Pharmacopoeia and recognized in Ayurveda and Siddha traditional medicine systems."
botanical: "Alangium salviifolium (L.f.) Wangerin"
family: "Cornaceae (Alangiaceae)"
sanskrit: ""
ayurvedicCategory: ""
whoStatus: "Not listed in WHO monographs on selected medicinal plants; documented in Indian Pharmacopoeia and recognized in Ayurveda and Siddha traditional medicine systems"
aliases: ["Alangium salviifolium (L.f.) Wangerin"]
tags: ["ayurveda", "herb", "single-herb", "general-herbs", "pharmacopoeia-listed", "clinical-evidence", "safety-concern"]
sources: ["Amidha Ayurveda Herb Database (700+ herbs, CC-BY-4.0)", "Web research (clinical trials, WHO monographs, safety databases)", "Ayurvedic Pharmacopoeia of India"]
productHints: []
words: 586
srcRel: "Ayurveda/Herbs/Single-Herbs/General-Herbs/Ankola/meta-analysis.md"
---
## Names and identification

| Language | Name |
|----------|------|
| English | Ankola |
| Latin/Botanical | *Alangium salviifolium (L.f.) Wangerin* |

## Key Phytochemical Constituents

- Alangine
- Ankorine
- Tubulosine
- Alangicine
- Salsoline
- Salviifosides A
- Beta-[sitosterol](/ayurveda-atlas/glossary/compounds-q-t/#sitosterol)
- [Emodin](/ayurveda-atlas/glossary/compounds-d-g/#emodin)
- Lamarckianin
- Alangimarine

## How does it work?

- Salviifosides A exerts COX-2 inhibitory effects, providing anti-inflammatory activity through selective cyclooxygenase-2 inhibition and reduction of prostaglandin E2 synthesis
- Tubulosine alkaloid demonstrates cytotoxic activity against cancer cells by inhibiting protein synthesis and inducing apoptosis through activation of intrinsic mitochondrial pathways
- Alangine and related [alkaloids](/ayurveda-atlas/glossary/compounds-1-a/#alkaloids) exhibit [antihypertensive](/ayurveda-atlas/glossary/pharmacology/#antihypertensive) effects through calcium channel blocking and ACE-inhibitory mechanisms in cardiovascular tissue

## Which traditional uses are supported by research?

- [Antidiabetic](/ayurveda-atlas/reference/antidiabetic/) activity validated through in vivo studies showing significant blood [glucose](/ayurveda-atlas/glossary/compounds-d-g/#glucose) reduction consistent with traditional Ayurvedic use for Prameha (diabetes)
- Antiarthritic and anti-inflammatory properties confirmed in preclinical models supporting traditional use for joint pain and rheumatism (Amavata) in Ayurveda
- Antivenomous properties (traditional use as snakebite remedy) partially validated through in vitro studies showing neutralization of venom phospholipase activity

## What do recent clinical trials show?

- **GCMS-based phytochemical profiling and in vitro pharmacological activities of plant Alangium salviifolium (L.f) Wang** (2024): All crude extracts showed considerable and comparable in vitro [antioxidant](/ayurveda-atlas/reference/antioxidant/) and antibacterial properties; GC-MS analysis identified multiple bioactive compounds including [terpenoids](/ayurveda-atlas/glossary/compounds-t-z/#terpenoids) and fatty acids.
- **Ankol plant (Alangium salvifolium) - The treasure trove of bioactives and medicinal potential** (2022): Comprehensive review documenting the plant's significant [anticancer](/ayurveda-atlas/reference/anticancer/), [antitumor](/ayurveda-atlas/glossary/pharmacology/#antitumor), antibacterial, antinociceptive, and anti-inflammatory properties across different plant parts.
- **Pharmacological and phytochemical studies of Alangium salvifolium Wang - A review** (2017): Evidence-based studies reported efficacy against hypertension, diabetes, epilepsy, cancer, inflammation, and ulcer, with multiple alkaloid classes contributing to diverse pharmacological effects.

## Recent safety updates

- Plant is purgative and emetic in nature; fruits and seeds should only be used under medical supervision due to potent alkaloid content
- Sub-chronic safety evaluation of aqueous leaf extract in rats showed no lethal effects at tested doses, but dose-dependent gastrointestinal effects were noted
- Contraindicated during pregnancy and breastfeeding due to emetic alkaloid content; avoid in patients with gastrointestinal bleeding or ulcers

## What is it made of?

### Key Active Markers

- Sitosterol
- Emodin
- Alangine
- Ankorine
- Tubulosine
- Alangicine
- Salsoline
- Salviifosides A
- Beta-
- Lamarckianin
- Alangimarine

**Analytical Methods:** HPLC fingerprinting, TLC (identity), LC-MS/MS (marker quantification)

## Dosage forms and preparation

**Dosage Forms:** Churna (seed/bark powder), Taila (seed oil — external use primarily), Lepa (paste for external application), Kashayam (bark decoction), Capsule

**Standard Dosage:** 1-3g powder twice daily (internal, with caution); external oil/paste as needed. Internal use only under practitioner supervision.

**Bioavailability:** Limited modern data. Alangium salviifolium (Ankola) contains isoquinoline alkaloids (alangine, alanginine) with moderate to good oral absorption. Emetine-like alkaloids have known bioavailability profiles from Ipecac studies. Seed oil constituents absorbed transdermally when applied topically. Caution: therapeutic index is narrow for alkaloid-rich preparations.

**Optimal Timing:** As directed by Ayurvedic physician. External applications typically twice daily to affected area.

**Standardized Extract:** Bark extract standardized to total alkaloid content (specify limit, typically 1-3% total alkaloids as alangine equivalent). Seed oil for external use — cold-pressed, unrefined.

**Shelf Life:** 2 years (powder); 3 years (seed oil); 2 years (capsules)

**Storage:** Clearly labeled RESTRICTED — not for unsupervised use. Cool, dry place, airtight containers. Below 25°C.

**Marker Compounds:** Alangine, Alanginine, Emetine, Cephaeline, Alangimarckine, Beta-sitosterol

### Extraction Methods

- Hydroalcoholic extraction (70:30) for alkaloid enrichment
- Cold-pressed seed oil for external preparations
- Aqueous bark decoction
- Acid-base extraction for alkaloid isolation and purification

### Synergistic Combinations

- With [Eranda](/ayurveda-atlas/herb/eranda/) taila for Vata disorders (external)
- With [Nirgundi](/ayurveda-atlas/herb/nirgundi/) for joint pain (external application)
- With [Haridra](/ayurveda-atlas/herb/haridra/) for wound healing paste
