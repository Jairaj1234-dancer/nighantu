---
title: "Pilu"
slug: "pilu"
kind: "herb"
section: "Ayurveda"
category: "Herbs"
subcategory: "Single-Herbs"
group: "Trees-Large-Plants"
answer: "Pilu (Salvadora persica) is a plant used in Ayurveda. Benzyl isothiocyanate is rapidly absorbed from GI tract but also rapidly metabolized. Fluoride content (natural) in stems provides topical dental mineralization effect. Chloride ions in extract enhance salivary production locally."
botanical: "Salvadora persica"
family: ""
sanskrit: ""
ayurvedicCategory: ""
whoStatus: ""
aliases: []
tags: ["ayurveda", "herb", "single-herb", "trees-large-plants", "pharmacopoeia-listed", "safety-concern"]
sources: ["Amidha Ayurveda Herb Database (700+ herbs, CC-BY-4.0)", "Ayurvedic Pharmacopoeia of India"]
productHints: []
words: 328
srcRel: "Ayurveda/Herbs/Single-Herbs/Trees-Large-Plants/Pilu/meta-analysis.md"
---
## Names and identification

| Language | Name |
|----------|------|
| English | Pilu |

## Ayurvedic pharmacology (Dravyaguna)

| Property | Value | Description |
|----------|-------|-------------|
| **Rasa** (Taste) | Katu, Tikta | Primary taste |
| **Guna** (Quality) | Laghu, Tikshna | Physical quality |
| **Virya** (Potency) | Ushna | Heating/Cooling effect |
| **Vipaka** (Post-digestive) | Katu | Post-digestive effect |

## Dosage forms and preparation

**Dosage Forms:** Churna (bark/fruit powder), Kwatha (decoction), Capsule, Tablet, Datun (chewing stick), Toothpaste/powder, Taila

**Standard Dosage:** 2-4 g bark powder daily; 15-20 mL decoction twice daily; 500-750 mg standardized extract twice daily; dental use as needed

**Bioavailability:** Salvadorine and other [alkaloids](/glossary/compounds-1-a/#alkaloids) from Salvadora persica (Pilu/Miswak) show moderate oral bioavailability (25-40%). Benzyl isothiocyanate is rapidly absorbed from GI tract but also rapidly metabolized. Fluoride content (natural) in stems provides topical dental mineralization effect. Chloride ions in extract enhance salivary production locally. Systemic absorption from chewing stick use is minimal but sufficient for dental benefits.

**Optimal Timing:** Chewing stick: morning before meals (traditional dental hygiene). Oral supplements: after meals. Toothpaste/powder: twice daily brushing.

**Standardized Extract:** Salvadora persica bark extract (8:1) standardized to salvadorine >1%, total [isothiocyanates](/glossary/compounds-g-l/#isothiocyanates) >0.5%, fluoride content documented. Stem aqueous extract for dental formulations with validated [antimicrobial](/reference/antimicrobial/) activity (MIC testing).

**Shelf Life:** Fresh chewing sticks: 1-2 weeks at ambient; Dried bark powder: 18-24 months; Extract: 24 months; Toothpaste: 24-36 months

**Storage:** Fresh sticks: cool, moist environment to prevent drying. Dried material: airtight containers, below 25 deg C, RH <50%. Volatile-containing extracts: sealed, amber containers, cool storage.

**Marker Compounds:** Salvadorine, Benzyl isothiocyanate, Salvadourea, Fluoride, Chloride, [Silica](/glossary/compounds-q-t/#silica), Beta-[sitosterol](/glossary/compounds-q-t/#sitosterol), Trimethylamine

### Extraction Methods

- Aqueous extraction of bark/root
- Hydroalcoholic extraction (50-60% ethanol)
- Steam distillation for volatile fraction
- Cold press of fruit for oil
- Water extraction of stem for dental preparations
- Methanol extraction for alkaloid profiling

### Synergistic Combinations

- Babool/Acacia (dental care formulation synergy)
- [Lavanga](/herb/lavanga/)/Clove ([analgesic](/reference/analgesic/) and antimicrobial dental combination)
- [Haridra](/herb/haridra/) (anti-inflammatory oral care)
- [Nimba](/herb/nimba/) (antimicrobial mouthwash combination)
- [Khadira](/herb/khadira/) (astringent gum health)
