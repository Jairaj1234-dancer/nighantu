---
title: "Kanmad Bhasma"
slug: "kanmad-bhasma"
kind: "herb"
section: "Ayurveda"
category: "Others"
subcategory: "Mineral-Metal-Preparations"
group: "Bhasmas"
answer: "Kanmad Bhasma is a bhasma, a mineral or metal preparation reduced to ash by repeated calcination. [Structural investigation of Ayurveda Lauha (Iron) Bhasma](https://pubmed.ncbi.nlm.nih.gov/36822148/). Journal of Ayurveda and integrative medicine. 2 further claims previously listed here could not be traced to a published paper and have been removed."
botanical: ""
family: ""
sanskrit: ""
ayurvedicCategory: ""
whoStatus: ""
aliases: ["Kanmad Bhasma", "KanmadBhasma"]
tags: ["ayurveda", "bhasma", "pharmacopoeia-listed", "clinical-evidence", "safety-concern"]
sources: ["Amidha Ayurveda Herb Database (700+ herbs, CC-BY-4.0)", "Web research (clinical trials, WHO monographs, safety databases)", "Ayurvedic Pharmacopoeia of India"]
productHints: []
words: 424
srcRel: "Ayurveda/Others/Mineral-Metal-Preparations/Bhasmas/Kanmad-Bhasma/meta-analysis.md"
---
## Names and identification

| Language | Name |
|----------|------|
| English | [Kanmad Bhasma](/herb/kanmad-bhasma/) |

## Key Phytochemical Constituents

- Magnetite (Fe3O4) - primary phase
- Hematite (Fe2O3) from oxidation during processing
- Fe2+/Fe3+ ratio optimized through calcination
- Trace carbon, sodium, zinc as minor elements
- Nanocrystalline iron oxide particles with magnetic properties

## How does it work?

- Superior iron bioavailability through magnetite nanoparticle form providing both Fe2+ and Fe3+ ions in optimal ratio for intestinal absorption and hemoglobin synthesis
- Potential magnetic hyperthermia therapy through nanocrystalline magnetite generating localized heat under alternating magnetic field for cancer cell killing
- Hematopoietic support through efficient iron delivery to bone marrow for red blood cell production, superior to ordinary iron preparations
- Metabolic enhancement through iron-dependent enzyme cofactor provision supporting cellular respiration, DNA synthesis, and energy metabolism

## Which traditional uses are supported by research?

- Iron-deficiency anemia (Pandu Roga): Iron supplementation efficacy well-established; magnetite form provides superior bioavailability per Ayurvedic and modern analysis
- Diabetes (Madhumeha): Traditional use supported by iron's role in [glucose](/glossary/compounds-d-g/#glucose) metabolism regulation and insulin sensitivity enhancement
- Chronic fevers (Jeerna Jwara): Anti-microbial and immune-supporting properties of iron compounds provide partial mechanistic validation

## What do recent clinical trials show?

- Tiwari MK, Singh A, Khooha A and others 2023. [Structural investigation of Ayurveda Lauha (Iron) Bhasma](https://pubmed.ncbi.nlm.nih.gov/36822148/). *Journal of Ayurveda and integrative medicine*. PMID [36822148](https://pubmed.ncbi.nlm.nih.gov/36822148/) · [doi:10.1016/j.jaim.2023.100690](https://doi.org/10.1016/j.jaim.2023.100690)  
  Lauha Bhasma from magnetite has great potential for noninvasive localized target killing of cancer cells via necrosis by application of alternating external magnetic field or photoelectron generation through X-rays.

*2 further claims previously listed here could not be traced to a published paper and have been removed. An absence here means we could not identify the source, not that no work exists.*

## Recent safety updates

- SUPERIOR TO ORDINARY IRON: Kanta [Loha Bhasma](/herb/loha-bhasma/) considered safest and most potent iron preparation in Ayurveda due to natural magnetite source
- DOSE LIMIT: Should not exceed 500 mg/day. Overuse can cause constipation, stomach upset, or iron toxicity.
- PROPER PREPARATION ESSENTIAL: Must undergo complete Shodhana, Marana, and Amruteekarana processes. Improperly prepared magnetite may contain heavy metal contaminants.
- IRON OVERLOAD MONITORING: Like all iron preparations, periodic hemoglobin, serum ferritin, and liver function monitoring recommended during prolonged use

## What is it made of?

### Mineral/Elemental Profile

- **Iron oxide (Fe2O3):** Major component (iron ore)
- **Iron (Fe):** >60%
- **Silicon (Si):** Trace
- **Aluminum (Al):** Trace

**Analytical Methods:** XRD, ICP-OES

## Dosage forms and preparation

**Dosage Forms:** Bhasma

**Standard Dosage:** 125–250 mg

**Bioavailability:** Moderate; mineral silicate form

**Optimal Timing:** With honey or specific formulation vehicle

**Standardized Extract:** Calcined mineral bhasma

**Shelf Life:** Indefinite

**Storage:** Airtight container

**Marker Compounds:** Calcium silicate, Iron oxide, [Magnesium](/glossary/compounds-l-o/#magnesium)

**Quality Parameters:** Standard bhasma tests, heavy metals within limits

**Vehicle (Anupana):** Honey
