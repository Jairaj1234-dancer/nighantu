---
title: "Brahmi"
slug: "brahmi"
kind: "herb"
section: "Ayurveda"
category: "Herbs"
subcategory: "Single-Herbs"
group: "Brain-Nervous-System"
answer: "Brahmi (Bacopa monnieri) is classified in Ayurveda as Medhya Rasayana (Nootropic Rejuvenative), from the Plantaginaceae family. Parts used: whole plant, leaf. Named after Brahma (creator god) for its intellect-enhancing properties. No dedicated WHO monograph currently published."
botanical: "Bacopa monnieri"
family: "Plantaginaceae"
sanskrit: ""
ayurvedicCategory: "Medhya Rasayana (Nootropic Rejuvenative)"
whoStatus: "No dedicated WHO monograph currently published. Listed in Ayurvedic Pharmacopoeia of India. Recognized in EU as a non-novel food (pre-1997 dietary supplement use). EMA monograph under development. Health Canada includes Bacopa in its Cognitive Function Products monograph. American Herbal Pharmacopoeia and Alternative Medicine Review have published independent monographs."
aliases: ["Bacopa monnieri"]
tags: ["ayurveda", "herb", "single-herb", "brain-nervous-system", "rasayana", "pharmacopoeia-listed", "clinical-evidence", "safety-concern"]
sources: ["Amidha Ayurveda Herb Database (700+ herbs, CC-BY-4.0)", "AgeAyurveda Product Catalog", "Classical Ayurvedic texts (Charaka Samhita, Sushruta Samhita, Bhavaprakasha Nighantu, Ashtanga Hridaya)", "Web research (clinical trials, WHO monographs, safety databases)", "Ayurvedic Pharmacopoeia of India"]
productHints: ["Natural Sleep Aid", "ADHD Ease", "Grow and Glow Hair Oil", "Nasja Oil"]
words: 1004
srcRel: "Ayurveda/Herbs/Single-Herbs/Brain-Nervous-System/Brahmi/meta-analysis.md"
---
## Names and identification

| Language | Name |
|----------|------|
| English | Brahmi |
| Common Names | [Water Hyssop](/herb/water-hyssop/), Brahmi, Bacopa |
| Hindi | Brahmi |
| Sanskrit | Brahmi, Saraswati |
| Latin/Botanical | *Bacopa monnieri* |

## Parts Used

- Whole plant
- Leaf

## Dosha Effects

| Dosha | Effect |
|-------|--------|
| **Vata** | Decreases |
| **Kapha** | Decreases |
| **Pitta** | Decreases |

## Key Phytochemical Constituents

- Bacosides A & B
- Bacopasaponins
- Brahmine
- Herpestine
- D-[mannitol](/glossary/compounds-l-o/#mannitolmannitol)

## Therapeutic Actions (Karma)

- Nootropic
- [Anxiolytic](/reference/anxiolytic/)
- Memory enhancer
- [Neuroprotective](/reference/neuroprotective/)
- [Antioxidant](/reference/antioxidant/)
- Anti-epileptic

## How does it work?

- Active compounds: Bacosides A and B (dammarane-type triterpenoid [saponins](/reference/saponins/)), betulinic acid, loliolide, asiatic acid, [quercetin](/reference/quercetinquercetin/).
- Neuronal repair: Bacoside aglycone units ([jujubogenin](/glossary/compounds-g-l/#jujubogenin)/pseudojujubogenin) repair damaged neurons, stimulate kinase activity, restore synaptic function, and improve nerve transmission.
- Acetylcholinesterase inhibition: [Quercetin](/reference/quercetinquercetin/), wogonin, [apigenin](/glossary/compounds-1-a/#apigenin), and bacopaside X dose-dependently inhibit AChE, with bacopaside X most potent.
- Antioxidant: Increases glutathione peroxidase (GPx), superoxide dismutase (SOD), and catalase; decreases malondialdehyde (MDA).
- Anti-inflammatory: Decreases IL-1beta, IL-6, and TNF-alpha; inhibits TNF-alpha and IL-6 release by activated N9 microglial cells.
- NF-kB modulation: Reduces NF-kB phosphorylation to suppress neuroinflammation.
- Alzheimer's pathology: Modulates GSK-3beta/Wnt/beta-catenin signaling and ameliorates mitochondrial dysfunction.
- Neurotrophic: Induces neurotrophic factors supporting neuronal growth and survival.

## How is it used traditionally?

Named after Brahma (creator god) for its intellect-enhancing properties. One of four Medhya Rasayanas in Charaka Samhita. Used for Smriti (memory), Medha (intellect), and Unmada (mental disorders).

## Where is it described in the classical texts?

- Charaka Samhita (Medhya Rasayana)
- Sushruta Samhita
- Bhavaprakasha Nighantu

## What do recent clinical trials show?

- Lopresti AL, Smith SJ 2025. [The Effects of a Bacopa monnieri Extract (Bacumen(®)) on Cognition, Stress, and Fatigue in Healthy Adults: A Randomized, Double-Blind, Placebo-Controlled Trial](https://pubmed.ncbi.nlm.nih.gov/41091332/). *Clinical drug investigation*. PMID [41091332](https://pubmed.ncbi.nlm.nih.gov/41091332/) · [doi:10.1007/s40261-025-01492-1](https://doi.org/10.1007/s40261-025-01492-1)  
  12-week supplementation did not show greater cognitive performance improvement vs placebo, but significantly reduced self-reported stress reactivity and fatigue after cognitive-demanding tasks.
- Dwivedi A, Anjali A, Narzari H and others 2026. [Efficacy of Bacopa monnieri (Linn.) on Cognitive Function and Alterations in Blood Metabolites in Patients With Amnestic Mild Cognitive Impairment and Early Alzheimer Disease: Protocol for an Exploratory Double-Blind, Randomized, Placebo-Controlled Trial](https://pubmed.ncbi.nlm.nih.gov/41740144/). *JMIR research protocols*. PMID [41740144](https://pubmed.ncbi.nlm.nih.gov/41740144/) · [doi:10.2196/82891](https://doi.org/10.2196/82891)  
  Exploratory trial protocol registered for evaluating efficacy on cognitive function and blood metabolite alterations in MCI and early Alzheimer's patients.
- Valotto Neto LJ, Reverete de Araujo M, Moretti Junior RC and others 2024. [Investigating the Neuroprotective and Cognitive-Enhancing Effects of Bacopa monnieri: A Systematic Review Focused on Inflammation, Oxidative Stress, Mitochondrial Dysfunction, and Apoptosis](https://pubmed.ncbi.nlm.nih.gov/38671841/). *Antioxidants (Basel, Switzerland)*. PMID [38671841](https://pubmed.ncbi.nlm.nih.gov/38671841/) · [doi:10.3390/antiox13040393](https://doi.org/10.3390/antiox13040393)  
  Comprehensive review confirmed Bacopa's neuroprotective effects through anti-inflammatory, antioxidant, neurotransmitter modulation, neurotrophic factor induction, and anti-apoptotic mechanisms.
- Sushma, Sahu MR, Murugan NA and others 2024. [Amelioration of Amyloid-β Induced Alzheimer's Disease by Bacopa monnieri through Modulation of Mitochondrial Dysfunction and GSK-3β/Wnt/β-Catenin Signaling](https://pubmed.ncbi.nlm.nih.gov/38143280/). *Molecular nutrition & food research*. PMID [38143280](https://pubmed.ncbi.nlm.nih.gov/38143280/) · [doi:10.1002/mnfr.202300245](https://doi.org/10.1002/mnfr.202300245)  
  Bacopa monnieri demonstrated significant amelioration of amyloid-beta-induced Alzheimer's pathology via mitochondrial function restoration and GSK-3beta/Wnt/beta-catenin pathway modulation.

*1 further claim previously listed here could not be traced to a published paper and has been removed. An absence here means we could not identify the source, not that no work exists.*

## Recent safety updates

- Generally well tolerated with high therapeutic index; adverse event rates similar to placebo in clinical trials.
- Most common side effects are gastrointestinal: increased stool frequency, nausea, abdominal cramps.
- Occasional reports of dry mouth, flu-like symptoms, headache, drowsiness (uncommon).
- No serious or severe adverse effects reported in clinical trials.
- Not linked to liver enzyme elevations or clinically apparent liver injury (LiverTox database).
- Drug interactions: May increase side effects of drugs metabolized by the liver (CYP [enzymes](/glossary/compounds-d-g/#enzymes)).
- Caution with cholinergic drugs (donepezil, rivastigmine, galantamine, tacrine) due to additive acetylcholine effects.
- May increase thyroid hormone levels; avoid with thyroid hormone medications and in hyperthyroidism.
- Contraindicated in pregnancy, stomach ulcers, intestinal/urinary blockages, slow heart rate, and lung disease.

## Recommended Dosage

Powder: 2-5g/day; Standardized extract (20% bacosides): 300-450mg/day

## Safety, contraindications and cautions

- May increase thyroid hormone levels
- GI effects at high doses
- Caution with CNS depressants

## What is it made of?

**Part(s) Analyzed:** Whole herb

### Triterpenoid Saponins (Bacosides)

- Bacoside A (8-12%)
- Bacoside B
- Bacopaside I-XII
- Bacopasaponin C
- Jujubogenin glycosides

### Alkaloids

- Brahmine
- Herpestine
- Nicotine (trace)

### Flavonoids

- Luteolin
- Apigenin
- Luteolin-7-glucoside

### Sterols

- Beta-sitosterol
- Stigmasterol
- Stigmastanol

**Analytical Methods:** HPLC-ELSD (bacosides), LC-MS/MS, UV spectrophotometry

## Dosage forms and preparation

**Dosage Forms:** Churna (powder), Tablet, Capsule, Swarasa (fresh juice), [Ghrita](/herb/ghrita/) (ghee preparation), Taila (oil), Syrup, Brahmi Rasayana

**Standard Dosage:** 3-6g powder twice daily; 300-450mg standardized extract (50% bacosides); 10-20ml juice; 5-10g Brahmi Ghrita; 5-10ml syrup

**Bioavailability:** Bacoside A shows moderate oral bioavailability (30-40%) with Tmax of 2-3 hours. Bacosides undergo hydrolysis to jujubogenin and pseudojujubogenin aglycones in the GI tract. Lipid-based formulations (Ghrita) significantly enhance bacoside bioavailability (2-3x). Phosphatidylserine complex of bacosides shows improved brain penetration. SNEDDS formulations achieve 3-4x enhancement. Chronic dosing (8-12 weeks) needed for cognitive effects.

**Optimal Timing:** Morning with milk or ghee for cognitive enhancement; consistent daily use for 8-12 weeks for full nootropic effect; Ghrita at bedtime for Vata pacification

**Standardized Extract:** Standardized extract with bacosides NLT 50% (by UV); NLT 25% bacoside A by HPLC. Whole plant extract 10:1 ratio for general preparations.

**Shelf Life:** 2 years (powder); 3 years (standardized extract capsule/tablet); 3 years (Ghrita); 1 year (syrup)

**Storage:** Cool, dry place away from light and moisture. Standardized extracts in well-sealed containers. Ghrita in glass jars at room temperature. Hygroscopic powder requires airtight packaging with desiccant.

**Marker Compounds:** Bacoside A (bacoside A3, bacopaside II, bacopasaponin C, jujubogenin isomer), Bacoside B, Bacosaponin C-F, [Luteolin](/glossary/compounds-l-o/#luteolin), [Apigenin](/glossary/compounds-1-a/#apigenin), Brahmine, Herpestine, Betulinic acid

### Extraction Methods

- Hydroalcoholic extraction (70:30 ethanol:water) for bacoside enrichment
- Aqueous extraction for traditional preparations
- Methanol extraction for research-grade saponin profiling
- Standardized extraction to 50% bacosides via solvent partitioning
- Traditional Ghrita preparation for lipid-based extraction

### Synergistic Combinations

- With Shankhapushpi for Medhya (cognitive enhancement) synergy
- With [Ashwagandha](/herb/ashwagandha/) for stress-related cognitive decline
- With [Jyotishmati](/herb/jyotishmati/) (Celastrus) for memory and learning
- With [Mandukparni](/herb/mandukparni/) (Gotu kola) for neural regeneration
- With [Vacha](/herb/vacha/) for speech and cognitive disorders
