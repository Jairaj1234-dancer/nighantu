---
title: "Guggulu"
slug: "guggulu"
kind: "formulation"
section: "Ayurveda"
category: "Herbs"
subcategory: "Classical-Formulations"
group: "Guggulus-Resin-Based"
answer: "Guggulu is a classical Ayurvedic guggulu, a resin-based preparation. The usual dose is 250-500 mg (Shuddha Guggulu) twice daily, as per AFI Part I. IUCN Red List: Critically Endangered (2015 assessment) with >80% population decline over 84 years due to unsustainable harvesting and habitat loss."
botanical: "Commiphora wightii (syn. C. mukul)"
family: "Burseraceae"
sanskrit: ""
ayurvedicCategory: "Medohara (Anti-obesity), Vata-Kaphahara"
whoStatus: "IUCN Red List: Critically Endangered (2015 assessment) with >80% population decline over 84 years due to unsustainable harvesting and habitat loss. Listed in Ayurvedic Pharmacopoeia of India. Government of India has banned export of the species. No specific WHO monograph, but recognized in traditional medicine pharmacopeias. Distribution restricted to dry regions of western India (Rajasthan, Gujarat) and adjoining Pakistan."
aliases: []
tags: ["ayurveda", "formulation", "guggulu", "pharmacopoeia-listed", "clinical-evidence", "safety-concern"]
sources: ["Amidha Ayurveda Herb Database (700+ herbs, CC-BY-4.0)", "Classical Ayurvedic texts (Charaka Samhita, Sushruta Samhita, Bhavaprakasha Nighantu, Ashtanga Hridaya)", "Web research (clinical trials, WHO monographs, safety databases)", "Ayurvedic Pharmacopoeia of India"]
productHints: []
words: 797
srcRel: "Ayurveda/Herbs/Classical-Formulations/Guggulus-Resin-Based/Guggulu/meta-analysis.md"
---
## Names and identification

| Language | Name |
|----------|------|
| English | Guggulu |
| Common Names | Indian Bdellium, Guggul |
| Hindi | Guggul |
| Sanskrit | Guggulu, Devadhupa |
| Latin/Botanical | *Commiphora wightii (syn. C. mukul)* |

## Parts Used

- Oleo-gum-resin

## Dosha Effects

| Dosha | Effect |
|-------|--------|
| **Vata** | Decreases |
| **Kapha** | Decreases |
| **Pitta** | May Increase |

## Key Phytochemical Constituents

- Guggulsterones (Z & E)
- Guggulipid
- Myrrhanol A
- Muscanone

## Therapeutic Actions (Karma)

- [Hypolipidemic](/ayurveda-atlas/glossary/pharmacology/#hypolipidemic)
- Anti-inflammatory
- Thyroid stimulant
- Anti-obesity
- Anti-arthritic
- Wound healing

## How does it work?

- Guggulsterone acts as an FXR (farnesoid X receptor) antagonist with IC50s of 17 uM (Z-form) and 15 uM (E-form), with gene-selective modulation capability.
- Inhibits NF-kB pathway: blocks LPS/IL-1b-induced ICAM-1 expression, NF-kB transcriptional activity, IkB phosphorylation/degradation, NF-kB DNA-binding activity, and IKK activity.
- Downregulates inflammatory gene products COX-2 and MMP-9 (key players in arthritis development).
- Regulates multiple transcription factors: NF-kB, STAT3, C/EBPalpha, androgen receptor, and glucocorticoid receptors.
- Complex resin composition includes volatile oils, terpenoid constituents, minerals, amino acids, [polysaccharides](/ayurveda-atlas/glossary/compounds-o-q/#polysaccharides), and steroidal derivatives.
- Hypolipidemic action mediated through FXR antagonism affecting bile acid metabolism and cholesterol homeostasis.

## How is it used traditionally?

Sushruta Samhita has an entire chapter (Guggulu Kalpa) dedicated to it. Described in Charaka as Medohara (fat-reducing). Base for many compound formulations.

## Where is it described in the classical texts?

- Charaka Samhita
- Sushruta Samhita (Guggulu Kalpa)
- Bhavaprakasha Nighantu
- Sharangadhara Samhita

## What do recent clinical trials show?

- **Phytopharmacology of Commiphora wightii (Guggulu): A Mini Review** (2025): Comprehensive review confirming anti-inflammatory, [antioxidant](/ayurveda-atlas/reference/antioxidant/), platelet-regulating, thyroid-stimulating, anti-cancer, anti-diabetic, hypolipidemic, [neuroprotective](/ayurveda-atlas/reference/neuroprotective/), [cardioprotective](/ayurveda-atlas/reference/cardioprotective/), [wound-healing](/ayurveda-atlas/reference/wound-healing/), and [antimicrobial](/ayurveda-atlas/reference/antimicrobial/) properties of guggulu.
- **Guggulsterone as a Modulator of Lipid Signaling and Metabolic Reprogramming in Cancer** (2025): Guggulsterone identified as a classical lipid-lowering phytosteroidal FXR antagonist that modulates lipid signaling and metabolic reprogramming relevant to cancer treatment.
- **The role of guggulsterone on the NF-kB pathway in inflammatory bowel disease: preclinical evidence** (2022): Guggulsterone significantly inhibits NF-kB signaling and attenuates expression of proinflammatory cytokines, showing therapeutic potential for IBD, Graves' orbitopathy, colorectal cancer, and atherosclerosis.
- **Guggulu and [Triphala](/ayurveda-atlas/herb/triphala/) for the Treatment of Hypercholesterolaemia: A Placebo-Controlled, Double-Blind, Randomised Trial** (2020): Double-blind RCT evaluating guggulu and [triphala](/ayurveda-atlas/herb/triphala/) combination for hypercholesterolemia management, providing modern clinical evidence for the traditional Ayurvedic application.
- **Dissecting the allosteric FXR modulation: a chemical biology approach using guggulsterone as a chemical tool** (2022): Guggulsterone acts as a peculiar FXR antagonist with gene-selective modulation capability, suggesting a nuanced pharmacological profile beyond simple receptor antagonism.

## Recent safety updates

- Decreases serum concentration of diltiazem and propranolol - monitor therapy when co-administered.
- Decreases effects of levothyroxine and desiccated thyroid by pharmacodynamic antagonism.
- 1 g guggulipid decreased bioavailability of propranolol and diltiazem in healthy male volunteers.
- Avoid with drugs highly dependent on CYP3A4 metabolism without medical guidance.
- Contraindicated in pregnancy, breastfeeding, and active liver disease.
- Side effects in clinical trials (2160 mg/day for 12 weeks): 10 of 22 individuals experienced gastrointestinal distress, fatigue, skin rash.
- Other reported side effects include headaches, nausea, vomiting, loose stools, diarrhea, belching, and hiccups.
- Used safely in clinical trials up to 24 weeks, with some evidence of safety up to 75 weeks.
- Commercial products often contain less or no claimed guggulsterone content - standardization of herbal products warranted.
- Recommend rechecking lipids and liver [enzymes](/ayurveda-atlas/glossary/compounds-d-g/#enzymes) at 8-12 weeks when using therapeutically.

## Recommended Dosage

Purified guggulu: 500mg-2g/day; Guggulsterones: 25mg three times daily

## Safety, contraindications and cautions

- Pregnancy
- Diarrhea
- Liver disease
- Thyroid medications (interaction)
- Estrogen-based contraceptives

## What is it made of?

**Part(s) Analyzed:** Oleo-gum resin

### Steroidal Compounds (Guggulsterones)

- Z-Guggulsterone (2-5%)
- E-Guggulsterone (1-3%)
- Guggulsterol I-V

### Terpenoids

- Myrrhanone A
- Myrrhanol A
- Cembrene A
- Allylcembrol

### Lignans

- Sesamin
- Guggullignan I-II

### Essential Oil Components

- Eugenol
- Cineole
- Limonene

**Analytical Methods:** HPLC-UV (guggulsterones E & Z), GC-MS (volatile fraction), TLC

## Dosage forms and preparation

**Dosage Forms:** [Shuddha Guggulu](/ayurveda-atlas/glossary/concepts-m-y/#shuddha-guggulu) (purified resin), Guggulu tablets, Capsules (modern)

**Standard Dosage:** 250-500 mg ([Shuddha Guggulu](/ayurveda-atlas/glossary/concepts-m-y/#shuddha-guggulu)) twice daily, as per AFI Part I

**Bioavailability:** Guggulsterones are lipophilic and moderately absorbed orally (~40-50%). Act as yogavahi (biocarrier) — enhance bioavailability of co-administered drugs. [Triphala](/ayurveda-atlas/herb/triphala/) kwath Shodhana adds gallic acid and [tannins](/ayurveda-atlas/reference/tannins/) that modulate absorption. Resin matrix provides sustained release.

**Optimal Timing:** After meals, twice daily

**Shelf Life:** 5 years from date of manufacture as per ASU guidelines

**Storage:** Store in airtight containers in a cool, dry place. Becomes very hard if dried excessively — maintain optimal moisture. Temperature not exceeding 30°C.

**Marker Compounds:** Z-Guggulsterone, E-Guggulsterone, Guggulsterol I, Guggulsterol II, Guggulsterol III, Myrrhanol A

**Quality Parameters:** Z-guggulsterone and E-guggulsterone content by HPLC (NLT 2.5% combined), total extractive value, acid value, volatile oil content, ash value, moisture (<12%), foreign organic matter (<2%), HPTLC fingerprint, microbial limits, heavy metals

**Vehicle (Anupana):** Warm water, or specific kashayam as directed

### Synergistic Combinations

- [Triphala](/ayurveda-atlas/herb/triphala/) Churna
- [Trikatu](/ayurveda-atlas/herb/trikatu/) Churna
- Specific herbs depending on formulation (Kanchanara, [Gokshura](/ayurveda-atlas/herb/gokshura/), Yogaraja, etc.)
